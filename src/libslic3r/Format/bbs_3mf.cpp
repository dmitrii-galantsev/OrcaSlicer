...
void PlateData::parse_filament_info(GCodeProcessorResult *result, const DynamicPrintConfig *config)
{
    ...
    for (auto it = ps.total_volumes_per_extruder.begin(); it != ps.total_volumes_per_extruder.end(); it++) {
        ...
        // Derive used_for_object/used_for_support from actual print volumes (more accurate than config-only).
        ...
        for (unsigned int i = 0; i < (unsigned int)plate_data_list.size(); ++i)
        {
            ...
            if (plate_data != nullptr && plate_data->is_sliced_valid) {
                ...
                for (auto it = plate_data->slice_filaments_info.begin(); it != plate_data->slice_filaments_info.end(); it++)
                {
                    ...
                    const std::string filament_nozzle_group_id = it->group_id < 0 ? std::to_string(nozzle_group_id) : std::to_string(it->group_id);
