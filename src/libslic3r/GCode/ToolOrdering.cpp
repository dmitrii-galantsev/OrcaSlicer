...
static std::set<int> get_filament_by_type(const std::vector<unsigned int>& used_filaments,
...
bool check_filament_printable_after_group(const std::vector<unsigned int>& used_filaments,
                                          ...
                                          const PrintConfig* print_config)
{
    ...
    for (unsigned int filament_id : used_filaments) {
        ...
        if (!(printable_status >> extruder_idx & 1)) {
            ...
            std::string error_msg     = _L("Grouping error: ") + filament_type + _L(" can not be placed in the ") + extruder_name +
                                    _L(" nozzle");
...
static FilamentChangeStats calc_filament_change_info_by_toolorder(const PrintConfig* config,
                                                                  const MultiNozzleUtils::LayeredNozzleGroupResult& group_result,
                                                                  ...
                                                                  const std::vector<std::vector<unsigned int>>& layer_sequences)
{
    ...
    // H2C BBL: Use NozzleStatusRecorder to track virtual nozzle states for H2C
    ...
    for (const auto& ls : layer_sequences) {
        for (const auto& filament : ls) {
            ...
            if (last_filament != -1 && last_filament != filament) {
                ...
                if (extruder_id < (int) flush_matrix.size() && last_filament < (int) flush_matrix[extruder_id].size() &&
                    filament < (int) flush_matrix[extruder_id][last_filament].size()) {
    ...
    for (auto& fv : flush_volume_per_filament) {
        float weight = config->filament_density.get_at(fv.first) * 0.001f * fv.second;
                                                                  ...
                                                                  const std::vector<std::vector<unsigned int>>& layer_sequences)
{
    ...
    std::vector<int> last_filament_per_extruder(max_extruder_id + 1, -1);
    ...
    for (const auto& ls : layer_sequences) {
        for (const auto& item : ls) {
            ...
            if (last_filament != -1 && last_filament != (int) item) {
                if (extruder_id < (int) flush_matrix.size() && last_filament < (int) flush_matrix[extruder_id].size() &&
                    item < (int) flush_matrix[extruder_id][last_filament].size()) {
                    flush_volume_per_filament[item] += flush_matrix[extruder_id][last_filament][item];
                    total_filament_change_count += 1;
                }
            }
    ...
    for (auto& fv : flush_volume_per_filament) {
        total_filament_flush_weight += config->filament_density.get_at(fv.first) * 0.001f * fv.second;
    }
    ret.filament_change_count = total_filament_change_count;
    ret.filament_flush_weight = (int) total_filament_flush_weight;
    return ret;
                                     ...
                                     const std::vector<std::pair<double, unsigned int>>& per_layer_extruder_switches)
{
    ...
    for (auto layer : object.layers()) {
        ...
        for (; it_per_layer_extruder_override != per_layer_extruder_switches.end() &&
               it_per_layer_extruder_override->first < layer->print_z + EPSILON;
             ++it_per_layer_extruder_override)
            extruder_override = (int) it_per_layer_extruder_override->second;

        // Store the current extruder override (set to zero if no overriden), so that
        // layer_tools.wiping_extrusions().is_overridable_and_mark() will use it.
            ...
            if (!layerm->perimeters.entities.empty()) {
                ...
                if (m_print_config_ptr) { // in this case print->config().print_sequence != PrintSequence::ByObject (see ToolOrdering constructors)
                    ...
                    for (const auto& eec : layerm->perimeters.entities) // let's check if there are nonoverriddable entities
                        if (!layer_tools.wiping_extrusions().is_overriddable_and_mark(dynamic_cast<const ExtrusionEntityCollection&>(*eec),
                                                                                      *m_print_config_ptr, object, region))
                            something_nonoverriddable = true;
                } else {
                    something_nonoverriddable = false;
                    for (const auto& eec : layerm->perimeters.entities) // let's check if there are nonoverriddable entities
                        if (!layer_tools.wiping_extrusions().is_obj_overriddable_and_mark(dynamic_cast<const ExtrusionEntityCollection&>(
                                                                                              *eec),
                                                                                          object))
                            something_nonoverriddable = true;
                }
                ...
                if (something_nonoverriddable) {
                    layer_tools.extruders.emplace_back((extruder_override == 0) ? region.config().outer_wall_filament_id.value :
                                                                                  extruder_override);
                    ...
                    if (layerCount == 0) {
                        firstLayerExtruders.emplace_back((extruder_override == 0) ? region.config().outer_wall_filament_id.value :
                                                                                    extruder_override);
                    }
                }
    ...
    for (auto support_layer : object.support_layers()) {
        ...
        for (const ExtrusionEntity* ee : support_layer->support_fills.entities) {
            ...
            if (er == erSupportMaterial || er == erSupportTransition)
                has_support = true;
            if (er == erSupportMaterialInterface)
                has_interface = true;
            ...
            if (extruder_support > 0 || !has_interface || extruder_interface == 0 || layer_tools.has_object)
                ...
                auto get_next_extruder = [&](int current_extruder, const std::vector<unsigned int>& extruders) {
                    std::vector<float> flush_matrix(
                        cast<float>(get_flush_volumes_matrix(object.print()->config().flush_volumes_matrix.values, 0,
                                                             object.print()->config().nozzle_diameter.values.size())));
...
void ToolOrdering::fill_wipe_tower_partitions(const PrintConfig& config, coordf_t object_bottom_z, coordf_t max_layer_height)
{
    ...
    for (int i = int(m_layer_tools.size()) - 2; i >= 0; --i)
        m_layer_tools[i].wipe_tower_partitions = std::max(m_layer_tools[i + 1].wipe_tower_partitions,
                                                          m_layer_tools[i].wipe_tower_partitions);
    ...
    // So, if there is a non-object layer starting with different extruder than the last one ended with (or containing more than one
    // extruder), we'll mark it with has_wipe tower.
...
void ToolOrdering::collect_extruder_statistics(bool prime_multi_material)
{
    ...
    if (prime_multi_material && !m_all_printing_extruders.empty()) {
        ...
        m_all_printing_extruders.erase(std::remove_if(m_all_printing_extruders.begin(), m_all_printing_extruders.end(),
    ...
    for (const LayerTools& layer_tool : m_layer_tools) {
        for (const unsigned int& filament : layer_tool.extruders) {
            ...
            if (has_non_support) {
                ...
                if (first_non_support_filament == (unsigned int) -1)
                    first_non_support_filament = filament;
...
float get_flush_volume(const std::vector<int>& filament_maps,
                       const std::vector<unsigned int>& extruders,
                       const std::vector<FlushMatrix>& matrix,
                       size_t nozzle_nums)
{
...
// H2C port: Extended signature with unprintable_volumes and nozzle_status params
// for GroupReorder's per-combo-range optimization. These params pass through to
// build_filament_group_context and refine_groups_by_Nozzle_State respectively.
// BBL ref: BambuStudio/src/libslic3r/GCode/ToolOrdering.cpp:1270 (build_filament_group_context)
    ...
    const std::vector<std::set<int>>& geometric_unprintables)
{
    ...
    bool has_multiple_nozzle         = std::any_of(print_config.extruder_max_nozzle_count.values.begin(),
                                                   print_config.extruder_max_nozzle_count.values.end(), [](int v) { return v > 1; });
    ...
    // Simple manual-mode path: only for printers WITHOUT a multi-nozzle carousel.
    // For H2C (extruder_nums > 1 AND has_multiple_nozzle), we MUST fall through to
    // the PAM path below which builds a full nozzle_list from
    // extruder_nozzle_stats/extruder_max_nozzle_count. The simple nozzle_list here
    // has only 2 entries (one per extruder) and would collapse all carousel
    // filaments into a single group_id, defeating multi-nozzle assignment.
    ...
    if (mode == FilamentMapMode::fmmNozzleManual) {
        ...
        auto manual_filament_map = print_config.filament_map.values;
        std::transform(manual_filament_map.begin(), manual_filament_map.end(), manual_filament_map.begin(), [](int v) { return v - 1; });
        auto nozzle_result = LayeredNozzleGroupResult::create(used_filaments, manual_filament_map, print_config.filament_volume_map.values,
                                                              print_config.filament_nozzle_map.values,
                                                              get_extruder_nozzle_stats(print_config.extruder_nozzle_stats.values),
                                                              print_config.nozzle_diameter.values.front());
        if (!nozzle_result) {
    ...
    for (size_t nozzle_id = 0; nozzle_id < extruder_nums; ++nozzle_id) {
        std::vector<float> flush_matrix(
            cast<float>(get_flush_volumes_matrix(print_config.flush_volumes_matrix.values, nozzle_id, extruder_nums)));
        std::vector<std::vector<float>> wipe_volumes;
        for (unsigned int i = 0; i < filament_nums; ++i)
            wipe_volumes.push_back(
                std::vector<float>(flush_matrix.begin() + i * filament_nums, flush_matrix.begin() + (i + 1) * filament_nums));
    ...
    std::vector<LayerPrintSequence> other_layers_seqs = get_other_layers_print_sequence(print_config.other_layers_print_sequence_nums.value,
    ...
    for (size_t idx = 0; idx < extruder_nums; ++idx) {
        // H2C port: extruder_nozzle_stats from the GUI is sometimes initialised with placeholder
        // entries that map every NozzleVolumeType to count 0 before the real printer reports its
        // nozzle inventory. Treat that case as "no stats yet" and fall back to extruder_max_nozzle_count.
        ...
        if (stats_unusable) {
            int fallback_count = (idx < print_config.extruder_max_nozzle_count.values.size()) ?
                                     print_config.extruder_max_nozzle_count.values[idx] :
                                     1;
            if (fallback_count <= 0)
                fallback_count = 1;
            // Phase D: this fallback over-reports for offline H2C. With max_nozzle_count=6
            // we tell the slicer all 6 rack slots are usable even if only 4 are loaded,
            // which produces multi-color G-code that may try to swap to a slot that's empty.
            // The right user-side fix is "Set Nozzle Count..." in the top menu (writes
            // extruder_nozzle_stats with force_keep_stat). Logging here so the failure mode
            // is observable in the field.
            BOOST_LOG_TRIVIAL(warning) << __FUNCTION__ << ": extruder " << idx
                                       << " has no usable nozzle stats; falling back to extruder_max_nozzle_count=" << fallback_count
                                       << " (over-reports if rack isn't fully populated)";
            nozzle_groups.emplace_back(format_diameter_to_str(print_config.nozzle_diameter.values[idx]),
                                       NozzleVolumeType(print_config.nozzle_volume_type.values[idx]), idx, fallback_count);
        } else {
            ...
            if (type == nvtHybrid) {
                ...
                nozzle_groups.emplace_back(format_diameter_to_str(print_config.nozzle_diameter.values[idx]), type, idx,
    ...
    if ((has_multiple_extruder && print->is_BBL_printer()) || has_multiple_nozzle) {
        ...
        FilamentGroupContext context;
        {
            ...
            context.speed_info.filament_change_time = print->config().machine_load_filament_time +
                                                      print->config().machine_unload_filament_time;
        ...
        if (has_multiple_nozzle) {
            if (mode == FilamentMapMode::fmmManual) {
                auto manual_filament_map = print_config.filament_map.values;
                std::transform(manual_filament_map.begin(), manual_filament_map.end(), manual_filament_map.begin(),
                               [](int v) { return v - 1; });
                ret = calc_filament_group_for_manual_multi_nozzle(manual_filament_map, context);
            } else if (mode == FilamentMapMode::fmmAutoForMatch) {
                // For-Match needs AMS filament info from a connected printer. CLI runs
                // never populate context.machine_info.machine_filament_info, so the
                // call would throw EmptyAmsFilaments. Fall through to AutoForFlush in
                // that case. Mirrors the GUI's FilamentMapDialog.cpp:171 fallback
                // (`mode == fmmAutoForMatch && !machine_synced ? fmmAutoForFlush : mode`).
                bool has_ams = false;
                for (const auto& vec : context.machine_info.machine_filament_info) {
                    if (!vec.empty()) {
                        has_ams = true;
                        break;
                    }
                ...
                if (has_ams) {
                    ...
                    BOOST_LOG_TRIVIAL(warning)
                        << __FUNCTION__
                        << ": fmmAutoForMatch requested but no AMS filament info available; falling back to fmmAutoForFlush";
...
{
    switch (mode) {
    case Slic3r::ToolOrdering::SingleExt: return m_stats_by_single_extruder;
    case Slic3r::ToolOrdering::MultiExtBest: return m_stats_by_multi_extruder_best;
    case Slic3r::ToolOrdering::MultiExtCurr: return m_stats_by_multi_extruder_curr;
    default: break;
    }
...
// H2C port: Collect per-layer filament lists and unprintable constraints.
// BBL ref: BambuStudio/src/libslic3r/GCode/ToolOrdering.cpp:1941-1953
ToolOrdering::LayerData ToolOrdering::collect_layer_and_unprintable_data()
{
    ...
    // H2C: filament_unprintable_volumes is only needed for O1D nozzle volume type constraints.
    // For H2C (single volume type per extruder) this is always empty.
    return data;
}
...
void ToolOrdering::reorder_extruders_for_minimum_flush_volume(bool reorder_first_layer)
{
    ...
    for (size_t nozzle_id = 0; nozzle_id < nozzle_nums; ++nozzle_id) {
        std::vector<float> flush_matrix(
            cast<float>(get_flush_volumes_matrix(print_config->flush_volumes_matrix.values, nozzle_id, nozzle_nums)));
        std::vector<std::vector<float>> wipe_volumes;
    ...
    {
        {
            ...
            m_print->update_filament_maps_to_config(FilamentGroupUtils::update_used_filament_values(print_config->filament_map.values,
                                                                                                    group_result->get_extruder_map(false),
                                                                                                    used_filaments),
                                                    FilamentGroupUtils::update_used_filament_values(print_config->filament_volume_map.values,
    ...
    // other_layers_seq: the layer_idx and extruder_idx are base on 1
    auto get_custom_seq = [&other_layers_seqs, &reorder_first_layer, &first_layer_filaments](int layer_idx,
                                                                                             std::vector<int>& out_seq) -> bool {
    ...
    Vortek::GroupReorder::reorder_extruders(*this, filament_lists, filament_maps, maps_without_group, layer_filaments, nozzle_flush_mtx,
                                            get_custom_seq, filament_sequences, nozzle_nums, map_mode, support_multi_nozzle);
    ...
    if (nozzle_nums > 1) {
        // always calculate the info by one extruder
        {
            ...
            reorder_filaments_for_multi_nozzle_extruder(filament_lists, result, layer_filaments, nozzle_flush_mtx, get_custom_seq,
                                                        &filament_sequences_one_extruder);
            m_stats_by_single_extruder = calc_filament_change_info_by_toolorder(print_config, maps_without_group, nozzle_flush_mtx,
                                                                                filament_sequences_one_extruder);
        }
        ...
        if (map_mode != fmmAutoForFlush) {
            std::vector<std::vector<unsigned int>> filament_sequences_one_extruder;
            // H2C TODO
            auto group_result_auto = get_recommended_filament_maps(m_print, layer_filaments, fmmAutoForFlush, physical_unprintables,
                                                                   geometric_unprintables);
            // reorder_filaments_for_multi_nozzle_extruder(
            // std::vector<int>filament_maps_auto = get_recommended_filament_maps(m_print, layer_filaments, fmmAutoForFlush,
            // physical_unprintables, geometric_unprintables);
            reorder_filaments_for_multi_nozzle_extruder(filament_lists, group_result_auto, layer_filaments, nozzle_flush_mtx,
                                                        get_custom_seq, &filament_sequences_one_extruder);
            m_stats_by_multi_extruder_best = calc_filament_change_info_by_toolorder(print_config, group_result_auto, nozzle_flush_mtx,
                                                                                    filament_sequences_one_extruder);
        }
    }
...
void ToolOrdering::assign_custom_gcodes(const Print& print)
{
    ...
    CustomGCode::Mode mode       = (num_filaments == 1)                 ? CustomGCode::SingleExtruder :
                                   print.object_extruders().size() == 1 ? CustomGCode::MultiAsSingle :
                                                                          CustomGCode::MultiExtruder;
    ...
    // Tool changes and color changes will be ignored, if the model's tool/color changes were entered in mm mode and the print is in non mm
    // mode or vice versa.
    ...
    for (auto it_lt = m_layer_tools.rbegin(); it_lt != m_layer_tools.rend(); ++it_lt) {
        ...
        if (custom_gcode.print_z > 0.5 * (print_z_below + lt.print_z)) {
            ...
            bool apply_color_change    = !ignore_tool_and_color_changes &&
                                      ...
                                      // BBS
                                      (color_change ? mode == CustomGCode::SingleExtruder ||
                                                          (custom_gcode.extruder <= int(num_filaments) &&
                                                           extruder_printing_above[unsigned(custom_gcode.extruder - 1)]) :
...
void WipingExtrusions::set_extruder_override(
    const ExtrusionEntity* entity, const PrintObject* object, size_t copy_id, int extruder, size_t num_of_copies)
{
...
float WipingExtrusions::mark_wiping_extrusions(const Print& print,
                                               ...
                                               float volume_to_wipe)
{
    ...
    if (!this->something_overridable || volume_to_wipe <= 0. || print.config().filament_soluble.get_at(old_extruder) ||
    ...
    for (int i = 0; i < (int) object_list.size() + (perimeters_done ? 0 : 1); ++i) {
        if (!perimeters_done && (i == (int) object_list.size() ||
                                 !object_list[i]->config().flush_into_objects)) { // we passed the last dedicated object in list
                ...
                if (is_infill_first != perimeters_done || wipe_into_infill_only) {
                    for (const ExtrusionEntity* ee : layerm->fills.entities) { // iterate through all infill Collections
                        ...
                        if ((!is_entity_overridden(fill, object, copy) &&
                             fill->total_volume() > min_infill_volume)) { // this infill will be used to wipe this extruder
                ...
                if (object->config().flush_into_objects && is_infill_first == perimeters_done) {
                    for (const ExtrusionEntity* ee : layerm->perimeters.entities) {
                        ...
                        if (is_overriddable(*fill, print.config(), *object, region) && !is_entity_overridden(fill, object, copy) &&
                            fill->total_volume() > min_infill_volume) {
                            set_extruder_override(fill, object, copy, new_extruder, num_of_copies);
                            if ((volume_to_wipe -= float(fill->total_volume())) <= 0.f)
                                // More material was purged already than asked for.
                                return 0.f;
            ...
            if (object->config().flush_into_support) {
                ...
                do {
                    ...
                    if (support_overriddable && !is_support_overridden(object) &&
                        !(object_config.support_interface_not_for_body.value && !support_intf_overriddable &&
                          (new_extruder == object_config.support_interface_filament - 1 ||
                           old_extruder == object_config.support_interface_filament - 1))) {
...
void WipingExtrusions::ensure_perimeters_infills_order(const Print& print)
{
    ...
    for (const PrintObject* object : print.objects()) {
        ...
        for (size_t copy = 0; copy < num_of_copies;
             ++copy) { // iterate through copies first, so that we mark neighbouring infills to minimize travel moves
            for (const LayerRegion* layerm : this_layer->regions()) {
                const auto& region = layerm->region();
                // BBS
                ...
                for (const ExtrusionEntity* ee : layerm->fills.entities) { // iterate through all infill Collections
                    ...
                    if (is_infill_first
                        // BBS
                        //|| object->config().flush_into_objects  // in this case the perimeter is overridden, so we can override by the
                        //last one safely
                        || lt.is_extruder_order(
                               lt.wall_extruder_id(region),
                               last_nonsoluble_extruder // !infill_first, but perimeter is already printed when last extruder prints
                                   || !lt.has_extruder(lt.sparse_infill_filament_id(
                                          region)))) // we have to force override - this could violate infill_first (FIXME)
                        set_extruder_override(fill, object, copy, (is_infill_first ? first_nonsoluble_extruder : last_nonsoluble_extruder),
                                              num_of_copies);
                    else {
                ...
                for (const ExtrusionEntity* ee : layerm->perimeters.entities) { // iterate through all perimeter Collections
                    auto* fill = dynamic_cast<const ExtrusionEntityCollection*>(ee);
                    if (is_overriddable(*fill, print.config(), *object, region) && !is_entity_overridden(fill, object, copy))
                        set_extruder_override(fill, object, copy, (is_infill_first ? last_nonsoluble_extruder : first_nonsoluble_extruder),
                                              num_of_copies);
                }
            }
        }
...
// Following function is called from GCode::process_layer and returns pointer to vector with information about which extruders should be
// used for given copy of this entity. If this extrusion does not have any override, nullptr is returned. Otherwise it modifies the vector
// in place and changes all -1 to correct_extruder_id (at the time the overrides were created, correct extruders were not known, so -1 was
// used as "print as usual"). The resulting vector therefore keeps track of which extrusions are the ones that were overridden and which
// were not. If the extruder used is overridden, its number is saved as is (zero-based index). Regular extrusions are saved as -number-1
// (unfortunately there is no negative zero).
const WipingExtrusions::ExtruderPerCopy* WipingExtrusions::get_extruder_overrides(const ExtrusionEntity* entity,
                                                                                  const PrintObject* object,
