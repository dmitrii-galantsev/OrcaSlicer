...
public:
    ...
    // Orca-only 3-arg overload: defaults filament_id to "" (callers in SelectMachine.cpp).
    MaterialItem(wxWindow *parent, wxColour mcolour, wxString mname) : MaterialItem(parent, mcolour, mname, std::string()) {}
    ~MaterialItem();
...
public:
    ...
    // Orca-only 3-arg overload: defaults filament_id to "" (callers in SyncAmsInfoDialog.cpp).
    MaterialSyncItem(wxWindow *parent, wxColour mcolour, wxString mname) : MaterialSyncItem(parent, mcolour, mname, std::string()) {}
    ~MaterialSyncItem();
    int  get_real_offset();
...
class AmsMapingPopup : public PopupWindow
{
    ...
    bool m_only_show_ext_spool = false; // Orca-only flag for ext-spool-only display mode.
...
public:
    ...
    // Orca-only: helper retained from pre-port for SelectMachineDialog to derive TrayData.
...
public:
    ...
    // Orca-only setter for the ext-spool-only display mode (SelectMachine path).
    void set_only_show_ext_spool(bool only) { m_only_show_ext_spool = only; }
