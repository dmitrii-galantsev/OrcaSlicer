...
{
public:
    wgtDeviceNozzleRackUpgradeDlg(wxWindow* parent, const std::shared_ptr<VortekNozzleRack> rack);

public:
    void UpdateRackInfo(const std::shared_ptr<VortekNozzleRack> rack);;
...
public:
    ...
    void UpdateExtruderNozzleInfo(const std::shared_ptr<VortekNozzleRack> rack);

    // rack nozzle
    void SetRackNozzleId(int rack_nozzle_id) { m_rack_nozzle_id = rack_nozzle_id; }
    int GetRackNozzleId() const { return m_rack_nozzle_id; }
    void UpdateRackNozzleInfo(const std::shared_ptr<VortekNozzleRack> rack);
