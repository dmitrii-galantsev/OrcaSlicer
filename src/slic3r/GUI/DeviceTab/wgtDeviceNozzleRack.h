...
public:
    void UpdateRackInfo(std::shared_ptr<VortekNozzleRack> rack);
    void Rescale();
...
public:
    void UpdateToolHeadInfo(const DevNozzle& extruder_nozzle, bool active, std::shared_ptr<VortekNozzleRack> rack);
    void Rescale();
...
public:
    void UpdateRackInfo(std::weak_ptr<VortekNozzleRack> rack);
    void Rescale();
...
private:
    std::weak_ptr<VortekNozzleRack> m_nozzle_rack;
    VortekNozzleRack::RackPos m_rack_pos       = VortekNozzleRack::RACK_POS_UNKNOWN;
    VortekNozzleRack::RackStatus m_rack_status = VortekNozzleRack::RACK_STATUS_UNKNOWN;
...
public:
    void UpdateRackPos(const std::shared_ptr<VortekNozzleRack>& rack);
    void Rescale();

private:
    void CreateGui();

    void UpdateRackPos(VortekNozzleRack::RackPos new_pos, VortekNozzleRack::RackStatus new_status, bool is_reading);
...
private:
    std::weak_ptr<VortekNozzleRack> m_rack;
    VortekNozzleRack::RackPos m_rack_pos       = VortekNozzleRack::RACK_POS_UNKNOWN;
    VortekNozzleRack::RackStatus m_rack_status = VortekNozzleRack::RACK_STATUS_UNKNOWN;
...
public:
    void Update(const std::shared_ptr<VortekNozzleRack> rack, bool on_rack = true); // on_rack is false means extruder nozzle
