...
void DevNozzleMappingCtrl::SetManualNozzleMappingByFila(int fila_id, int nozzle_pos_id)
{
    ...
    try {
        // m_nozzle_mapping_json is null until an MQTT mapping ack populates it (see line 278).
        // Treat null/non-array as an empty mapping; grow with -1 sentinel for unmapped fila_ids.
        std::vector<int> mapping;
        if (m_nozzle_mapping_json.is_array()) {
            mapping = m_nozzle_mapping_json.get<std::vector<int>>();
        }
        if (static_cast<int>(mapping.size()) <= fila_id) {
            mapping.resize(fila_id + 1, -1);
        }
        if (mapping[fila_id] != nozzle_pos_id) {
