...
// Orca-only helper retained from pre-BBL-port AmsMappingPopup; SelectMachineDialog uses it.
std::vector<TrayData> AmsMapingPopup::parse_ams_mapping(const std::map<std::string, DevAms*, NumericStrCompare>& amsList)
{
    std::vector<TrayData> tray_data_list;
    for (auto ams_iter = amsList.begin(); ams_iter != amsList.end(); ams_iter++) {
        ...
        for (auto tray_iter = ams_group->GetTrays().cbegin(); tray_iter != ams_group->GetTrays().cend(); tray_iter++) {
            DevAmsTray* tray_data = tray_iter->second;
            TrayData td;
            td.id = ams_indx * AMS_TOTAL_COUNT + atoi(tray_data->id.c_str());
            if (!tray_data->is_exists) {
                td.type = EMPTY;
            } else if (!tray_data->is_tray_info_ready()) {
                td.type = THIRD;
            } else {
                td.type = NORMAL;
